<?php
/**
 * friends.php — Feed filtered to mutual follows only.
 * Lives at: /project/filter/friends.php
 */
...
<div class="flexDesk" id="alignment">
    ...
    <main>
        ...
        $stmt3 = $myPDO->prepare('SELECT following_id FROM following WHERE user_id = ?');
        $stmt3->execute([$uid]);
        $followingIds = $stmt3->fetchAll(PDO::FETCH_COLUMN);

        $stmt2 = $myPDO->prepare('SELECT user_id FROM following WHERE following_id = ?');
        $stmt2->execute([$uid]);
        $followerIds = $stmt2->fetchAll(PDO::FETCH_COLUMN);

        $friendIds = array_values(array_intersect($followingIds, $followerIds));

        if (empty($friendIds)) {
            echo '<p>You have no mutual friends yet. Follow someone and wait for them to follow back!</p>';
        } else {
            foreach ($allPosts as $row) {
                ...
                if (!in_array($u_id, $friendIds)) continue;
